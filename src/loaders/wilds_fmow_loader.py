...
class WildsFMoWDataset(Dataset):
    def __init__(self, subset: str):
        self.dataset = get_dataset(dataset="fmow", download=True)
        # Need to normalize lilke this for pre-trained efficient net from pytorch
        self.normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        self.dataset = self.dataset.get_subset(
            subset,
