...
class LinearScalarizationHarness(SimpleHarness):
    def _do_train(self):
        ...
        preference_ray = torch.from_numpy(preference_ray[self.cfg.train.preference_ray_idx]).to(dtype=torch.float)
...
class MultiProcessHarness(SimpleHarness):
    ...
    def _do_test(self):
        with ProcessPoolExecutor(max_workers=len(self.cfg.train.gpus), mp_context=multiprocessing.get_context('spawn')) \
                as executor:
